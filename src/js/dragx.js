...
Vue.directive('dragx', (el, binding, vnode) => {
  ...
  function getDirection(e) {
    ...
    // 无语，这个 value 其实是 v-dragx="1" 这里的 1 ，但是默认为 {} 所以 下面的选择一直为 true
    if (binding.value) {
  ...
  el.onmousedown = function (e) {
    ...
    document.onmousemove = function (edom) {
      ...
      if (isMove && cfg.canDrag) {
        ...
        let deltX = targetPageX - data.startX;
        let deltY = targetPageY - data.startY;
        ...
        setConstraint(data);
        // data.left = targetPageX;
        // data.top = targetPageY - data.startY + data.height;
      }
      if (cfg.dirctDom) {
        if (cfg.canResize) {
